  ...
  Josh.Shell = function(config) {
    ...
    var self = {
      ...
      templates: {
        ...
        bad_command: _.template('<div>command not found:&nbsp;<%=cmd%></div>'),
      ...
      render: function() {
        ...
        if(!cursor) {
          $(id(_input_id) + ' .input .cursor').html('&nbsp;').css('background-color', '#859900')
        } else {
          $(id(_input_id) + ' .input .cursor').text(cursor).css('background-color', '#859900')
        }
    ...
    function blinkCursor() {
      ...
      root.setTimeout(function() {
        ...
        if(_cursor_visible) {
          $(id(_input_id) + ' .input .cursor').css('background-color', '#859900');
        } else {
          $(id(_input_id) + ' .input .cursor').css('background-color', '');
        }
        blinkCursor();
      }, _blinktime);
    }
